<!DOCTYPE html>
<html lang="en">
<head>
   <meta charset="UTF-8">
   <meta http-equiv="X-UA-Compatible" content="IE=edge">
   <meta name="viewport" content="width=device-width, initial-scale=1.0">
   <title>Profile</title>
   <style>
    *{
   font-family: 'Nunito', sans-serif;
   margin:0; 
   padding:0;
   box-sizing: border-box;
   outline: none; border:none;
   text-decoration: none;
}

html{
   font-size: 62.5%;
   overflow-x: hidden;
}

.message{
   position: absolute;
   top:0;
   max-width: 1200px;
   padding:1.5rem 2rem;
   display: flex;
   align-items: center;
   justify-content: space-between;
   gap:1.5rem;
   z-index: 1000;
   background-color: #ffffffa5;
   margin-left: 30.3vw;
   width: 40vw;
   border-bottom-left-radius: 2vw;
   border-bottom-right-radius: 2vw;
}

.message span{
   color: #262D3F; 
   font-size: 2rem;
}

.message i{
   color: #262D3F;
   font-size: 2.5rem;
   cursor: pointer;
}

.message i:hover{
   color: rgb(199, 68, 68);
}

.title{
   padding:1.5rem 1rem;
   text-align: center;
   font-size: 3rem;
   text-transform: uppercase;
   color:#000000;
   font-family: Trade Winds, 'Source Sans Pro';
   font-weight: 100;
   letter-spacing: 0.4vw;
}

.btn,
.delete-btn,
.option-btn{
   display: block;
   width: 80%;
   border-radius: .5rem;
   color:white;
   font-size: 1.8rem;
   cursor: pointer;
   padding:1rem 3rem;
   text-transform: capitalize;
   text-align: center;
   margin-top: 1rem;
   height: 2.5vw;
   margin-left: 2.5vw;
}

.btn{
   background-color: #131f2f;
}

.delete-btn{
   background-color: #131f2f;
}

.option-btn{
   background-color: #131f2f;
}

.btn:hover,
.delete-btn:hover,
.option-btn:hover{
   background-color: white;
   color: black;
}


.form-container{
   display: flex;
   align-items: center;
   justify-content: center;
   padding:2rem;
   min-height: 100vh;
}

.form-container form{
   width: 44.1rem;
   border-radius: 1.5rem;
   background-color:  #ffffffa5;
   padding:2rem;
   text-align: center;
}

.form-container form h3{
   font-size: 2.8rem;
   font-weight: 100;
   margin-bottom: 1rem;
   font-family: Trade Winds, 'Source Sans Pro';
   color: #1e1e1e;
   text-transform: uppercase;
}

.form-container form .box{
   width: 100%;
   border-radius: .5rem;
   padding:1rem 1rem;
   font-size: 1.5rem;
   color: black;
   background-color: WHITE;
   margin:1rem 0;
   height: 2.2vw;
   font-family: Nobile, 'Source Sans Pro';
}

.form-container form p{
   font-size: 1.8rem;
   color: black;
   padding:1rem 0;
   margin-top: 1vw;
}

.form-container form p a{
   color:#0f2e99;
   margin-left: 0.4vw;
}

.form-container form p a:hover{
   text-decoration: underline;
}


.profile-container{
   display: flex;
   align-items: center;
   justify-content: center;
   padding:2rem;
   min-height: 100vh;
}

.profile-container .profile{
   width: 40rem;
   border-radius: .5rem;
   background-color: black;
   padding:2rem;
   text-align: center;
}

.profile-container .profile img{
   height: 20rem;
   width: 20rem;
   border-radius: 50%;
   object-fit: cover;
   margin-bottom: .5rem;
}

.profile-container .profile h3{
   font-size: 2.5rem;
   padding:.5rem 0;
   color: white;
}

.update-profile-container{
   display: flex;
   align-items: center;
   justify-content: center;
   padding:2rem;
   min-height: 100vh;
}

.update-profile-container form{
   width: 61rem;
   border-radius: 1.5rem;
   background-color: #ffffffa5;
   padding:2rem;
   text-align: center;
}

.update-profile-container form .flex{
   text-align: left;
   display: flex;
   flex-wrap: wrap;
   justify-content: space-between;
   
}

.update-profile-container form .flex .inputBox{
   width: 49%;
}

.update-profile-container form img{
   height: 15rem;
   width: 15rem;
   border-radius: 50%;
   margin-bottom: 1rem;
}

.update-profile-container form .flex .inputBox span{
   font-size: 2rem;
   color: #262D3F;
   display: block;
   padding-top: 1rem;
}

.update-profile-container form .flex .inputBox .box{
   width: 100%;
   padding:1.2rem 1.4rem;
   border-radius: .5rem;
   font-size: 1.8rem;
   color: #262D3F;
   margin:1rem 0;
   height: 2.7vw;
   background-color: white;
}

.update-profile-container form .flex-btn{
   width: 35vw;
   margin-left: 2vw;
}

.container{
   height: 100vh;
   width: 100vw;
   position: relative;
}

.container>img{
   position: absolute;
   z-index: -1;
   height: 100vh;
   width: 100vw;
}
/* Navbar */
.nav-section{
   width: 12vw;
   height: 100%;
   position: absolute;
   z-index: 999;
}

 
.logo{
   display: flex;
   flex-direction: column;
   margin-top: 4vh;
   margin-left: 2.5vw;
}
 
.logo img{
   width: 3.5vw;
   height: 2.7vw;
   border-radius: 0px;
   
}
 
.logo button{
   width: 6vw;
   height: 1.5vw;
   border: none;
   border-radius: 1vw;
   margin-top: 1.5vh;
   font-size: 0.85vw;
   background-color: #262D3F; 
   color: rgb(255, 255, 255);
   cursor: pointer;
   margin-left: -1.2vw;
   padding-bottom: 0.5vh;
}
 
.logo button a{
   text-decoration: none;
   color: #ffffff;
}
 
.menu{
   height: 50vh;
   display: flex;
   flex-direction: column;
   margin-top: 20vh;
   margin-left: 1vw;
}
 
.menu ul{
   list-style: none;
   font-size: 1.3vw;
   margin-left: -1vw;
}
 
.menu ul li p{
 position: absolute;
 white-space: nowrap;
 margin-top: -4.8vw; 
 margin-left: 6vw;
 visibility: hidden;
 opacity: 0;
 color: #FFF;
}
.menu ul li i{
   margin-bottom: 7vh;
   opacity: 0.8;
   margin-left: 3.7vw;
   color: #FFF; 
   transition: all linear 0.1s;
}
 
.menu ul li i:hover{
   opacity: 1;
   font-size: 1.4vw;
}

.error{
   font-size: 1vw;
   float: left;
}

.error2{
   font-size: 1vw;
   float: left;
   width: 100%;
}

#fpwd{
   margin-left: -15vw;
   font-size: 1.2vw;
   color: #0f2e99;
}
.text-danger{
    color: red;
}
   </style>
</head>
<body>
<div class="container">
    <img src="{{ asset('images/Logo-Background/hotstar-blue-bg.jpg') }}">
    <section class="update-profile-container">
        <form action="{{ route('update_profile') }}" method="post" enctype="multipart/form-data">
            @csrf
            <img src="{{ Auth::user()->image ? asset('User_profiles/' . Auth::user()->image) : asset('User_profiles/default.webp') }}" alt="profile image" style="object-fit:cover" />
            <div class="flex">
                <div class="inputBox">
                    <span>Username :</span>
                    <input type="text" name="update_name" value="{{ old('update_name', $user->name) }}" class="box">
                    @error('update_name')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror

                    <span>Your Email :</span>
                    <input type="email" name="update_email" value="{{ old('update_email', $user->email) }}" class="box">
                    @error('update_email')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror

                    <span>Update Your Pic :</span>
                    <input type="file" name="update_image" class="box">
                    @error('update_image')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>

                <div class="inputBox">
                    <span>Old Password :</span>
                    <input type="password" name="update_pass" placeholder="Enter previous password" class="box">
                    @error('update_pass')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror

                    <span>New Password :</span>
                    <input type="password" name="new_pass" placeholder="Enter new password" class="box">
                    @error('new_pass')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror

                    <span>Confirm Password :</span>
                    <input type="password" name="confirm_pass" placeholder="Confirm new password" class="box">
                    @error('confirm_pass')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
            </div>
            <input type="submit" value="Update Profile" class="btn u-btn" style="background-color: #121212; color:white; height:2.8vw">
            <a href="{{ auth()->user()->role === 'admin' ? route('admin') : route('my_space') }}" class="delete-btn" style="background-color: white; color:#121212; height:2.3vw;"><p style="font-size:1.1vw; margin-top:-0.3vw">Go Back</p></a>
        </form>
    </section>
</div>
</body>
</html>
